    ...
    def encontra_tu_libro():
        ...
        if user_number.strip() != '':
            try:
                ...
                st.markdown(f'- Primer género favorito: ** {primer_genero} **')
                st.markdown(f'- Segundo género favorito: ** {segundo_genero} **')
                st.markdown(f'- Páginas promedio por libro: ** {paginas_promedio} **')
            except ValueError:
